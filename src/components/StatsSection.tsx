...
const stats = [
  { icon: Users, value: "200+", label: "Happy Clients", bg: "card-coral", emoji: "🤝" },
  { icon: Target, value: "$5M+", label: "Revenue Generated", bg: "card-sky", emoji: "💰" },
  { icon: Award, value: "50+", label: "Awards Won", bg: "card-mint", emoji: "🏆" },
  { icon: Rocket, value: "98%", label: "Client Retention", bg: "card-lavender", emoji: "🚀" },
];
      ...
      <div className="max-w-6xl mx-auto" ref={ref}>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-5 mb-20">
          {stats.map((stat, i) => (
            ...
            >
              <span className="text-4xl block mb-3 group-hover:scale-125 transition-transform duration-300">{stat.emoji}</span>
        ...
        >
          ...
          <div className="relative h-80 md:h-96">
            ...
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-card rounded-3xl shadow-[var(--shadow-elevated)] p-8 w-3/4 text-center z-10">
              ...
              <div className="text-2xl font-display font-extrabold text-foreground">Since 2018</div>
              <div className="text-muted-foreground text-sm mt-1">Building digital empires</div>
            </div>
          </div>

          {/* Right - text */}
          <div>
            <span className="pill-badge bg-peach text-coral mb-4">💡 Who We Are</span>
            ...
            <p className="text-muted-foreground mt-6 text-lg leading-relaxed">
              We're a team of creative misfits, data nerds, and strategy obsessives who believe 
              every brand has an untold story worth sharing with the world.
            </p>
            <p className="text-muted-foreground mt-4 leading-relaxed">
              From startups to Fortune 500s, we've helped brands of all sizes break through 
              the noise and connect with audiences that matter.
            </p>
            <div className="mt-8 flex flex-wrap gap-3">
              {["Data-Driven", "Creative First", "ROI Focused", "Always On"].map((tag) => (
                <span key={tag} className="px-4 py-2 rounded-full bg-muted text-foreground font-semibold text-sm">
                  {tag}
                </span>
              ))}
            </div>
